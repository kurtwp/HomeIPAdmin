...
def page_layout(title: str = "Home Lab Manager"):
    ...
    # Prevent white flash in Firefox — set background on <html> before page renders
    bg_color = "#1a1a2e" if is_dark else "#f5f5f5"
    ui.add_head_html(f'<style>html{{background:{bg_color};transition:none}}</style>')
        ...
        with ui.row().classes("items-center gap-4"):
            ...
            with ui.button("Discovery").props("flat color=white no-caps").classes("nav-btn"):
                with ui.menu():
                    ...
                    ui.separator()
                    ui.menu_item("Firewall Rules", lambda: ui.navigate.to("/firewall"))
                    ui.menu_item("DHCP Leases", lambda: ui.navigate.to("/dhcp-leases"))
                    ui.separator()
